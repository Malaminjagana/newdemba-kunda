(function () {
    ...
    function galleryMarkup(record) {
        return (record.images || []).slice(1).map(function (image, index) {
            return '<img src="' + escapeHtml(image) + '" alt="' + escapeHtml(record.name + ' historical photograph ' + (index + 2)) + '" loading="lazy" data-cover-image>';
        }).join('');
    }
    ...
    function showDetail(record) {
        ...
        const isWell = key === 'wells';
        const detailLabel = isPond ? 'Pond Archive' : isFarmingRecord ? (record.category === 'forest' ? 'Forest Archive' : 'Farmland Archive') : isWell ? 'Water Source Archive' : 'Family Archive';
        const detailAction = isPond ? 'Close pond details' : isFarmingRecord ? 'Close landscape details' : isWell ? 'Close water source details' : 'Close family details';
        const location = isPond ? '' : '<p class="collection-neighbourhood">' + escapeHtml(isLegacyRecord ? record.neighborhood : record.location) + '</p>';
        const compound = isPond || isFarmingRecord || isWell ? '' : '<p class="collection-compound">' + escapeHtml(record.compoundNumber) + '</p>';
